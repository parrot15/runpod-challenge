  ...
  return (
    <nav className="bg-slate-950 p-4">
      <ul className="flex justify-center items-center space-x-4">
        <li className={`rounded-lg ${isActive('/') ? 'bg-purple-700' : 'hover:bg-white hover:bg-opacity-30 duration-300'}`}>
          <Link href="/" className='p-3 text-white'>
            <FontAwesomeIcon icon={faHome} className='mr-2' />
            Generate
          </Link>
        </li>
        <li className={`rounded-lg ${isActive('/library') ? 'bg-purple-700' : 'hover:bg-white hover:bg-opacity-30 duration-300'}`}>
